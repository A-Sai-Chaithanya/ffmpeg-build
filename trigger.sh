...
if [[ "$(docker images -q saichaithanya/ffmpeg-build-toolchain:v1 2> /dev/null)" == "" ]]; then
    if [[ "$(docker manifest inspect saichaithanya/ffmpeg-build-toolchain:v1 2> /dev/null)" == "" ]]; then
        docker build -t saichaithanya/ffmpeg-build-toolchain:v1 ./build-tools &&
        docker image push saichaithanya/ffmpeg-build-toolchain:v1
    else
